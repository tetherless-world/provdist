...
changelog = open('changelog_json.html', 'w')
...
print "Added Column"
for i in range(0,v2_sheet.ncols):
        v1_index = index_convert(i)
...
print "Added Row"
new_row = list(set([i.value for i in v2_sheet.col(0)])- set(indicator_map.keys()))
for i in range(3,v2_sheet.nrows):
        if v2_sheet.cell(i,0).value in new_row:
                #print i, v2_sheet.cell(i,0).value
		out = u'''        <tr id="AddChange%i" about="v2:Attribute%i">
          <td>%i</td>
          <td property="http://www.w3.org/2000/01/rdf-schema#label">%s</td>
          <script type="application/ld+json">
'''%(i, i, i, v2_sheet.cell(i,0).value)
                changelog.write(out.encode('utf8'))
                json1 = {
"@context":context,
"@type":"vo:AddChange" ,
